...
return { enqueue, flush, loadQueueCount, clearSynced, get count() { return queueCount; } };
})();
...
sheet.style.animation = “sheetDown 0.25s var(–ease-out) forwards”;
sheet.style.setProperty(”–ease-out”, “cubic-bezier(0.16, 1, 0.3, 1)”);
setTimeout(() => {
overlay().style.display = “none”;
...
minusBtn.addEventListener(evt, (e) => { e.preventDefault(); minusBtn.classList.add(“pressing”); startPress(-step); });
plusBtn.addEventListener(evt,  (e) => { e.preventDefault(); plusBtn.classList.add(“pressing”);  startPress(+step); });
});
[“touchend”, “touchcancel”, “mouseup”, “mouseleave”].forEach(evt => {
minusBtn.addEventListener(evt, () => { minusBtn.classList.remove(“pressing”); stopPress(); });
plusBtn.addEventListener(evt,  () => { plusBtn.classList.remove(“pressing”);  stopPress(); });
});
...
let taraDisplay  = null;
let netoDisplay  = null;
...
// Store chip selector reference to reset later
...
records.push({ ...record, id: Date.now() }); // approximate id for UI
records = await DB.all(DB.STORES.sucio);
...
```
document.getElementById("sucio-clear-btn").addEventListener("click", clearAll);
```
...
// Check if product already exists → sum instead of duplicate
const existing = records.find(r => r.producto === productoSeleccionado);
if (existing) {
...
renderList();
updateBadge();
document.getElementById(“limpio-clear-btn”).addEventListener(“click”, clearAll);
}
...
// ============================================================
const HotelModule = (() => {
// State: { [type_capacity]: count }
let state = {};

function stateKey(type, capacity) { return `${type}_${capacity}`; }
...
CONFIG.tiposHotel.forEach(type => {
  ...
  CONFIG.capacidadesHotel.forEach(cap => {
    ...
    // Long press on count → slider modal
    ...
    // Buttons — with long press acceleration
    let pressTimer = null;
    let pressInterval = null;
    ...
    ["touchstart", "mousedown"].forEach(e => {
      minusBtn.addEventListener(e, (ev) => { ev.preventDefault(); minusBtn.classList.add("pressing"); startPress(-1); });
      plusBtn.addEventListener(e,  (ev) => { ev.preventDefault(); plusBtn.classList.add("pressing");  startPress(+1); });
    });
    ["touchend", "touchcancel", "mouseup", "mouseleave"].forEach(e => {
      minusBtn.addEventListener(e, () => { minusBtn.classList.remove("pressing"); stopPress(); });
      plusBtn.addEventListener(e,  () => { plusBtn.classList.remove("pressing");  stopPress(); });
    });
...
document.getElementById("cfg-flush-btn").addEventListener("click", async () => {
  if (!navigator.onLine) { Toast.warning("Sin conexión — datos guardados localmente"); return; }
...
function init() {
window.addEventListener(“online”,  () => { update(); Toast.success(“Conexión restaurada”); Haptic.success(); });
window.addEventListener(“offline”, () => { update(); Toast.warning(“Sin conexión — datos guardados”); Haptic.heavy(); });
update();
}
...
// Auto-start if returning user (optional UX tweak)
// Could check if there’s existing data and auto-advance
}
